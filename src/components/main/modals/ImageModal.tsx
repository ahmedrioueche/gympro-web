    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
      ...
      <div className="bg-white dark:bg-gray-800 text-light-text-primary dark:text-dark-text-primary rounded-2xl shadow-2xl max-w-2xl w-full p-2 relative">
        ...
        <div className="flex justify-between space-x-4">
          <CustomButton text={dict[selectedLanguage].retake} className="flex-1" onClick={() => onRetake()} />

          <CustomButton type="secondary" className="flex-1" text={dict[selectedLanguage].use} onClick={() => onUse()} />
        </div>
      </div>
    </div>
  );
